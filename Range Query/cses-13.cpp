...
void construct2(int i,int ss,int se,int tree[][4000],int tree2[][4000]){
    if(ss==se){
        for(int j=1;j<(4*n);j++){
            tree2[i][j] = tree[ss][j];
        }
...
void update2(int i,int ss,int se,int index,int tree[][4000],int tree2[][4000]){
    if(ss==se && ss==index){
        for(int j=1;j<(4*n);j++){
            tree2[i][j] = tree[ss][j];
        }
...
int getans2(int i,int ss,int se,int a,int b,int c,int d,int tree[][4000],int tree2[][4000]){
    if(ss >= a && se <= c){
        int ans = getans(1,1,n,b,d,tree[ss]);
        return ans;
    }
